...
export default function DiscoverySection({ onItemAdded }: DiscoverySectionProps) {
  ...
  const fetchRecommendations = async (pageNumber = 1) => {
    setLoading(true);
    await fetchLibrary();
    const data = await getRecommendations(pageNumber);
  ...
  const handlePageChange = (newPage: number) => {
    ...
    // Remonter en haut de la section pour un meilleur confort visuel
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '1rem 0' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '15px' }}>
        ...
        >
          {loading ? 'Mise à jour...' : '🔄 Mélanger & Actualiser'}
        </button>
      </div>
      ...
      {!loading && recommendations.length > 0 && (
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px', marginTop: '3.5rem' }}>
            ...
            <button 
              onClick={() => handlePageChange(page + 1)} 
              disabled={loading || recommendations.length < 5}
              className="btn btn-secondary"
              style={{ padding: '0.55rem 1.2rem', borderRadius: '10px', fontWeight: 'bold' }}
            >
              Suivant ➡️
